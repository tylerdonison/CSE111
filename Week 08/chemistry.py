def main():
    ...
    chemical_formula = input("Please insert the chemical formula (ie, H2O): ")
    # Get a mass in grams from the user.
    grams = float(input("Please insert how much mass in grams of the said chemical formula: "))
    ...
    formula_mass = compute_molar_mass(symbol_quantity_list, periodic_table)
    # Compute the number of moles of the sample.
    moles = grams / formula_mass
    # Compute atoms
    atoms = compute_atoms(moles)
    # Print the molar mass.
    print(f"The molar mass is {formula_mass:.2f}.")
    # Print the number of moles.
    print(f"The number of moles is {moles:.2f}.")
    # Print the number of atoms.
    print(f"The number of atoms is {atoms}")
...
def compute_molar_mass(symbol_quantity_list, periodic_table):
    ...
    total_mass = 0
    if symbol_quantity_list != []:  
        symbols, quantity = map(list, zip(*symbol_quantity_list))
        list_length = len(symbols)
        for x in range(list_length):
            element_list = periodic_table[symbols[x]]
            symbol_mass = (element_list[ATOMIC_MASS_INDEX])
            total_mass = total_mass + (symbol_mass * quantity[x])
    return total_mass

def compute_atoms(moles):
        atoms = moles*6.02214076*10**23
        return atoms
